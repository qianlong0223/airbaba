...
{
    [self payWithOrder:order amount:amount title:@"充值" completation:comp];
...
+ (void)payWithOrder:(NSString *)order amount:(float)amount title:(NSString *)title completation:(void(^)(BOOL resp))comp
{
    ...
    NSString *totalFee  = [NSString stringWithFormat:@"%.0f", amount*100];                                         //交易价格1表示0.01元，10表示0.1元
    NSString *tradeNO   = [self generateTradeNO];                       //随机字符串变量 这里最好使用和安卓端一致的生成逻辑
    NSString *orderNo   = order;   //随机产生订单号用于测试，正式使用请换成你从自己服务器获取的订单号
    NSString *notifyUrl = [NSString stringWithFormat:@"http://112.74.200.84/api/callback/wxpay"];// 交易结果通知网站此处用于测试，随意填写，正式使用时填写正确网站
    ...
    [[session POST:kUrlWechatPay parameters:string success:^(NSURLSessionDataTask * _Nonnull task, id  _Nonnull responseObject) {
        //  输出XML数据
